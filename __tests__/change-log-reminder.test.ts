...
describe('Change Log Reminder Test', () => {
  ...
  let customPrMessage
  const fileList = {data: [{filename: 'file'}]} as OctokitResponse<
    PullsListFilesResponseData
  >
  const commentsList = {
    data: [{body: 'some comment', id: 1}]
  } as OctokitResponse<IssuesListCommentsResponseData>

  beforeEach(() => {
    jest.spyOn(octokitMock.pulls, 'listFiles').mockResolvedValue(fileList)
    jest
      .spyOn(octokitMock.issues, 'listComments')
      .mockResolvedValue(commentsList)
    jest
      .spyOn(octokitMock.issues, 'createComment')
      .mockResolvedValue({} as OctokitResponse<IssuesCreateCommentResponseData>)
    jest.spyOn(octokitMock.issues, 'deleteComment')
  })
  ...
  describe("change log is in pr doesn't create comment", () => {
    ...
    it('and deletes previous reminders', async () => {
      commentsList.data[0].body =
        '@peterjgrainger your pull request is missing a changelog!'
      fileList.data[0].filename = 'change_log/v1.2.3/change.yml'
      await changeLogReminder(octokitMock, contextMock, coreMock)
      expect(octokitMock.issues.deleteComment).toHaveBeenCalled()
    })
  })
  ...
  describe("doesn't create a comment", () => {
    ...
    it('no pull request sets to failed', async () => {
      contextMock.payload.pull_request = undefined
      await changeLogReminder(octokitMock, contextMock, coreMock)
  ...
  describe('setFailed called', () => {
    it('when listFiles api fails', async () => {
      jest
        .spyOn(octokitMock.pulls, 'listFiles')
